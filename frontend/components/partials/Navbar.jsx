...
export default function Navbar() {
  ...
  const handleLogout = () => {
    ...
    // Small delay to ensure background request is scheduled, then navigate to login
    setTimeout(() => {
      try {
        if (typeof window !== "undefined") {
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="logout-confirm-title" className="relative bg-white rounded-2xl shadow-2xl p-4 w-full max-w-[18rem] text-center pointer-events-auto">
            ...
            {loadingLogout ? (
              <div className="flex flex-col items-center gap-3 mt-4">
                <svg className="animate-spin h-8 w-8 text-[#0B1C39]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
                </svg>
                <p className="text-sm text-[#0B1C39] font-medium">Logging out...</p>
              </div>
            ) : (
